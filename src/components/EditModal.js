...
export const EditModal = ({content,imageId,imageContent,videoContent,id,setModalOpen}) => {
    ...
    return(
        ...
        flex flex-col gap-3 justify-center items-center rounded-md shadow-md shadow-primary_bg w-7/12 z-30">
            <img src={imageId} alt="avatar" className="w-12"/>        
            <input   className='p-4 border-0 outline-none  text-lg bg-secondary_bg text-color w-full text-center flex justify-center' onInput={(e) => setEditDetails({...editDetails,content:e.target.value})} placeholder="What's Happening?" value={editDetails.content}/>
                
             <div className="relative">
                 {
                editDetails?.imageContent && <img src={editDetails.imageContent} alt="post" className='flex justify-center w-4/4 rounded-sm relative'/>
            }
            {
                editDetails?.videoContent && <video className='w-full relative' autoPlay><source src={editDetails.videoContent}></source></video>
            }
        ...
        : <></>}
            ...
            <label for="edit" className="cursor-pointer" >
